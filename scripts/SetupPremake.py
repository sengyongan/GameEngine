...
class PremakeConfiguration:
    ...
    @classmethod
    def Validate(cls):
        if (not cls.CheckIfPremakeInstalled()):#是否安装
    ...
    @classmethod
    def CheckIfPremakeInstalled(cls):#是否存在exe
        premakeExe = Path(f"{cls.premakeDirectory}/premake5.exe");#"./vendor/premake/bin"
        if (not premakeExe.exists()):#不存在就安装
